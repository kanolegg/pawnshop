...
if (isset($_GET['id'])) {
	$id = $_GET['id'];
	$product = $core->get('products', ['id' => $id])[0];
...
if (isset($_POST) && !empty($_POST)) {
	...
	header('Location: /admin/category/'.$category);
}
				...
				<div class="container">
					...
					<div class="rounded shadow-sm border p-3" id="content">
						<form method="POST" class="row g-3">
							<div class="col-md-4">
								<input type="text" class="form-control" placeholder="Название" required name="title" value="<?=@$product['title']?>">
							</div>
							<div class="col-md-4">
								<input type="text" class="form-control" placeholder="Артикул" required name="vendor_code" value="<?=@$product['vendor_code']?>">
							</div>
							<div class="col-md-4">
								<select name="category" class="form-select">
									<option selected disabled value="">Категория</option>
									<option value="rings" <?=($product['category']==='rings')?'selected':''?>>Кольца</option>
									<option value="earrings" <?=($product['category']==='earrings')?'selected':''?>>Серьги</option>
									<option value="chains" <?=($product['category']==='chains')?'selected':''?>>Цепочки</option>
									<option value="pendant" <?=($product['category']==='pendant')?'selected':''?>>Подвески</option>
									<option value="bracelets" <?=($product['category']==='bracelets')?'selected':''?>>Браслеты</option>
									<option value="cufflinks" <?=($product['category']==='cufflinks')?'selected':''?>>Запонки</option>
									<option value="watch" <?=($product['category']==='watch')?'selected':''?>>Часы</option>
								</select>
							</div>
							<div class="col-md-3">
								<input type="text" class="form-control" placeholder="Цена" required name="price" value="<?=@$product['price']?>">
							</div>
							<div class="col-md-3">
								<input type="text" class="form-control" placeholder="Проба" name="content" value="<?=@$product['content']?>">
							</div>
							<div class="col-md-3">
								<select name="metal" class="form-select">
									<option selected disabled value="">Металл</option>
									<option value="Золото" <?=($product['metal']==='Золото')?'selected':''?>>Золото</option>
									<option value="Серебро" <?=($product['metal']==='Серебро')?'selected':''?>>Серебро</option>
								</select>
							</div>
							<div class="col-md-3">
								<input type="text" class="form-control" placeholder="Вес" name="weight" value="<?=@$product['weight']?>">
							</div>
							<div class="col-md-12">
								<textarea name="description" placeholder="Описание" rows="3" class="form-control"><?=@$product['description']?></textarea>
							</div>
							<div class="col-12">
